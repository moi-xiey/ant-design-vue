<template>
  <page-layout title="单号：234231029431" logo="https://gw.alipayobjects.com/zos/rmsportal/nxkuOJlFJuAUhzlMTCEe.png">
    ...
    <v-row slot="extra" class="status-list">
      <v-col :xs="12" :sm="12">
    ...
    <template slot="action">
      <v-button-group style="margin-right: 4px;">
      ...
      <v-button type="primary">主操作</v-button>
    </template>

    <v-card :bordered="false" title="流程进度">
      <v-steps :direction="isMobile() && 'vertical' || 'horizontal'" :current="1" progressDot>
        <v-step title="创建项目">
        </v-step>
        <v-step title="部门初审">
        </v-step>
    ...
    <v-card style="margin-top: 24px" :bordered="false" title="用户信息">
      <detail-list>
    ...
    <v-card style="margin-top: 24px" :bordered="false" title="用户近半年来电记录">
      <div class="no-data">
        <v-icon type="frown-o"/>
        暂无数据
      </div>
    ...
    >
      ...
      >
        <template
          slot="status"
          slot-scope="status">
          <v-badge :status="status | statusTypeFilter" :text="status | statusFilter"/>
        </template>
      </v-table>
      <v-table
        v-if="activeTabKey === '2'"
...
<script>
  ...
  import DetailList from '../../component/tools/detail-list';
